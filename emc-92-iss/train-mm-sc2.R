...
## docker build -t docker.synapse.org/syn7237116/emc-92-iss:version1 .
## docker images
## docker login docker.synapse.org
## docker push docker.synapse.org/syn7237116/emc-92-iss:version1
...
source(paste0(base.dir, "R/emc-92.R"))
...
## Train the EMC-92 model.
y <- anno.tbl[,c("sample.id", "D_PFS", "D_PFS_FLAG")]
colnames(y) <- c("ID", "time", "event")
rownames(y) <- y$ID
y <- na.omit(y)

res <- emc.92.entrez(X = as.matrix(expr), y = y, threshold = NULL)$obj
...
## NB: the training is no different than that for EMC-92, since we
## use the same threshold for EMC-92, but simply modulate the
## binarized "highriskflag" based on ISS.

threshold <- res$threshold
mapping <- emc.92.probe.to.entrez.mapping()
