...
## docker build -t docker.synapse.org/syn7237116/uams-17:version3 .
## docker images
## docker login docker.synapse.org
## docker push docker.synapse.org/syn7237116/uams-17:version3
...
source(paste0(base.dir, "R/uams-17.R"))
...
## Train the UAMS-17 model.
y <- anno.tbl[,c("sample.id", "D_PFS", "D_PFS_FLAG")]
colnames(y) <- c("ID", "time", "event")
rownames(y) <- y$ID
y <- na.omit(y)

res <- uams.17.entrez(X = as.matrix(expr), y = y, threshold = NULL)$obj
...
mapping <- uams.17.probe.to.entrez.mapping()
