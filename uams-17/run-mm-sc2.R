...
source(paste0(docker.image.base, "R/uams-17.R"))
...
## Subset the expression data sets to only have the genes in the UAMS-17
## data set (and that are in common between all data sets).

## Get the Entrez genes in the model.  This is defined in R/uams-17.R
## model.entrezs <- uams.17.probe.to.entrez.mapping()
## However, we don't have access to the internet (required of biomart in uams.17.probe.to.entrez.mapping).
...
## as invoked during uams.17.probe.to.entrez.mapping).  So, instead call that during the training
## phase (where we do have access to the internet) and store in the metadata.

## res <- uams.17.entrez(X = as.matrix(expr), y = NULL, threshold = threshold)
...
eset <- ExpressionSet(as.matrix(X))
coefficients <- uams.17.get.coefficients()
res <- uams.17.gene(eset, mapping = mapping, coefficients = coefficients, anno = y, threshold = threshold, standardize.using.training.data = FALSE)
