...
source(paste0(docker.image.base, "R/uams-5.R"))
...
## However, we don't have access to the internet (required of biomart in uams.5.probe.to.entrez.mapping).
...
## as invoked during uams.5.probe.to.entrez.mapping).  So, instead call that during the training
...
eset <- ExpressionSet(as.matrix(X))
coefficients <- uams.5.get.coefficients()
res <- uams.5.gene(eset, mapping = mapping, coefficients = coefficients, anno = y, threshold = threshold)
